...
async def test_subscription_rls_quota_and_expired_write_restriction() -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        tenant_a = Tenant(
            name=f"Subscription A {suffix}", domain=f"sub-a-{suffix}.test"
        )
        tenant_b = Tenant(
            name=f"Subscription B {suffix}", domain=f"sub-b-{suffix}.test"
        )
        ...
        subscription_a = TenantSubscription(
            tenant_id=tenant_a.id, plan_id=starter.id, status="ACTIVE", is_current=True
        )
        ...
        session.add_all(
            [
                Student(
                    ...
                    first_name="Quota",
                    last_name=str(index),
                    admission_number=f"{suffix}-{index}",
                )
        ...
        await session.execute(
            text(f'GRANT "{role_name}" TO "{session_user}" WITH ADMIN OPTION')
        )
        await session.execute(text(f'GRANT app_runtime TO "{role_name}"'))
        await session.commit()
        try:
            ...
            await session.execute(text(f'REVOKE app_runtime FROM "{role_name}"'))
        ...
        current = await session.scalar(
            ...
            .where(TenantSubscription.id == subscription_a.id)
            .with_for_update()
        )
        ...
        assert (
            await service.require_feature(
                tenant_a.id, FeatureCode.STUDENTS_MANAGE, write=False
            )
        ).can_read
        with pytest.raises(HTTPException) as status_error:
            await service.require_feature(
                tenant_a.id, FeatureCode.STUDENTS_MANAGE, write=True
            )
