...
# Patch Frontend
with open("src/components/forms/LoginForm.tsx", "w", encoding="utf-8") as f:
    f.write(login_form)

# Patch Backend
backend_schema_path = "../backend/app/schemas/auth.py"
if os.path.exists(backend_schema_path):
    with open(backend_schema_path, "r", encoding="utf-8") as f:
        content = f.read()
    
    content = content.replace("EmailStr", "str")
    
    with open(backend_schema_path, "w", encoding="utf-8") as f:
        f.write(content)
    print("UI and Backend patched successfully.")
else:
    print("UI patched. Could not locate backend schema.")